...
namespace SSDL
{
    public class ExcelCreationforXdata
    {
        ...
        public void Writehead(System.Data.DataTable dt, string title, List<KeyValuePair<string, string>> titleblockvalue = null, string desc = "", string fileName = "")
        {
            ...
            if (mExportDoc != null)
            {
                ...
                if (desc != "")
                {
                    ...
                    Range Getrange = SheetName.Range[string.Concat(columnstring, "5"), Missing.Value];
                    //bool bIsMerged = Getrange.MergeCells;
                    //MessageBox.Show(bIsMerged.ToString()+","+ Getrange.MergeArea.Address);
                    writeoncell(1, 5, "Hospital Description :");
                    //writeoncell(3, 5, desc,textbold:true);
                    writeoncell(Getrange.MergeArea.Columns.Count+1, 5, desc, textbold: true);
                }
                int i = 7;
                ...
                //20-Oct-2018//fulltitle = System.IO.Path.Combine(path, "BOM_" + title + "_" + DateTime.Now.Year + "_" + DateTime.Now.Month + "_" + DateTime.Now.Day + ".xlsx");
                fulltitle = System.IO.Path.Combine(path,  title +  "_BOM.xlsx");
                ...
                try
                {
                    ...
                    mExportDoc.SaveAs(fulltitle);
                    mExportDoc.ExportAsFixedFormat(XlFixedFormatType.xlTypePDF, System.IO.Path.Combine(path, title + "_BOM.pdf"));
                    //No Need to close
                    mExportDoc.Close();
                    mExcelApp.Quit();
        ...
        public void WriteonTimerollup(System.Data.DataTable dt, string title, string totalTime = "0", string desc = "", string revision = "")
        {
            ...
            //20-Oct-2018//string fulltitle = System.IO.Path.Combine(path, "TIME_ROLLUP_" + title + "_" + DateTime.Now.Year + "_" + DateTime.Now.Month + "_" + DateTime.Now.Day + ".xlsx");

            string fulltitle = System.IO.Path.Combine(path, title + "_" + DateTime.Now.Year + "_" + DateTime.Now.Month + "_" + DateTime.Now.Day + "_TIME_ROLLUP.xlsx");
        ...
        public void writeoncell(int col, int row, string towrite, bool textbold = false, bool addBorder = false, bool isNumber = false)
        {
            ...
            Getrange.VerticalAlignment = XlVAlign.xlVAlignCenter;
            Getrange.Font.Name = "Arial";
            Getrange.Font.Size = 16;
