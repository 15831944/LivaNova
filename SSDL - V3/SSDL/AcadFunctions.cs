...
namespace SSDL
{
    ...
    {
        [CommandMethod("GenerateProdDWG")]
        public void GenerateProdDwg()
        {
            View.GenerateProdDwg dwg = new View.GenerateProdDwg(System.Diagnostics.Process.GetCurrentProcess().MainWindowHandle);
            dwg.ShowDialog();
        }
        ...
        public void PrintBOM()
        {
            ...
            {
                BOM od = new BOM();
                //od.strBomName = od.getProjectDesc();
                ...
                od.strBomName = _desc.textBoxdesc.Text;
        ...
        public void PrintTimeRollUp()
        {
            ...
            string projectDesc = "";// od.getProjectDesc();//20-Oct-2018
            bool result = od.timerollupfunctionality(out sumresult, out drgnumber, out SurfaceAreaSum, out projectDesc, true, readDesc: false, divideBy60: false, updateDWGAttr: false); //02-05-2018
        ...
        public void Initialize()
        {
            if (Autodesk.AutoCAD.ApplicationServices.Application.Version.Major >= 23)//only till 2018
            {
                ...
                Database db = doc.Database;
                Editor ed = doc.Editor;
                ed.WriteMessage("\nSSDL.dll is incompatible with this version of AutoCAD.");
    ...
    public class BOM
    {
        ...
        public System.Collections.ObjectModel.ObservableCollection<View.clsTimeRollUp> timeRollupCol;
        ...
        public void SendXml(bool readDesc = true, string description = "", string sumresult = "", string drgnumber = "", string SurfaceAreaSum = "")
        {
            try
            {
                ...
                if (readDesc)
                {
                    bool result = timerollupfunctionality(out sumresult, out drgnumber, out SurfaceAreaSum, out description, false, divideBy60: false);
                    if (!result) return;
                }
                drgnumber = drgnumber.Replace("P_", "").Trim();
                //20-Oct-2018
        ...
        public static void xmlbuilder(System.Data.DataTable bom, string totaltimerollup, string itemcode = "", string description = "", string Bomcode = "", string drawingno = "", string SurfaceArea = "")
        {
            ...
            //if (strText.Equals("GenerateProdDWG", StringComparison.InvariantCultureIgnoreCase))
            //{
                fulltitle = System.IO.Path.Combine(xmlLocation,  "P_" + drawingno +"_"+ DateTime.Now.ToString("yyyy_MM_dd") + "_XML.xml");
            //}
        ...
        public void updateBlockData(Dictionary<int, string> dicItemNo)
        {
            try
            {
                Document doc = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument;
                                ...
                                foreach (ObjectId attId in attCol)
                                {
                                    try
                                    {
                                        using (AttributeReference attRef = (AttributeReference)tr.GetObject(attId, OpenMode.ForWrite))
                                        {
                                            ...
                                            {
                                                //attRef.TextString = dicItemNo.FirstOrDefault(a => a.Value.ToString().Equals(partNo, StringComparison.InvariantCultureIgnoreCase)).Key.ToString();
                                                int nItem = dicItemNo.FirstOrDefault(a => a.Value.ToString().Equals(partNo, StringComparison.InvariantCultureIgnoreCase)).Key;
                                                if (nItem != 0)
                                                {
                                                    attRef.TextString = nItem.ToString();
                                                }
                                                break;
                                            }
                                        }
        ...
        public void Generate(bool placeInAutoCAD = false, bool ExcelOutput = false, bool Timerollup = false, bool AutocadLayerwise = false)
        {
            ...
            if (AutocadLayerwise)
            {
                ...
                //---------------------
                System.Data.DataTable dtBase = Get_Drawingdata(includeLenAttr: false,updateDwgAttr:false);
                System.Data.DataTable dtBaseExcel = new System.Data.DataTable();
                dtBaseExcel = dtBase.DefaultView.ToTable(false, "Itemno", "Quantity", "Partno", "Description");
                //------Added at 30-10-2018 by Sundari------------------
                DataView dvBaseNew = dtBaseExcel.DefaultView;
                AlphanumComparator<string> comparer = new AlphanumComparator<string>();
                System.Data.DataTable dtBaseNew = dvBaseNew.Table.AsEnumerable().OrderBy(x => x.Field<string>("Partno"), comparer).CopyToDataTable();

                //dv.Sort = "Partno";
                dtBaseExcel = dtBaseNew;// dv.ToTable();
                int nVal = 1;
                Dictionary<int, string> dicItemNo = new Dictionary<int, string>();
                foreach (DataRow dr in dtBaseExcel.Rows)
                {
                    dr["Itemno"] = nVal;
                    dicItemNo.Add(nVal, dr["Partno"].ToString());
                    nVal++;
                }
                updateDwgData(dicItemNo);
                //-------------------------------------
                ...
                foreach (DataRow datarow in layerlist.Rows)
                {
                    ...
                    //------Added at 30-10-2018 by Sundari------------------
                    //DataView dvNew = dtnewExcel.DefaultView;
                    //AlphanumComparator<string> comparer = new AlphanumComparator<string>();
                    //System.Data.DataTable dtNew = dvNew.Table.AsEnumerable().OrderBy(x => x.Field<string>("Partno"), comparer).CopyToDataTable();
                    ...
                    {
                        dr["ItemNo"] = dicItemNo.FirstOrDefault(a => a.Value.ToString().Equals(dr["Partno"].ToString(), StringComparison.InvariantCultureIgnoreCase)).Key;
                    }
                    DataView dvItem = dtnewExcel.DefaultView;
                    dvItem.Sort = "Itemno";
                    dtnewExcel = dvItem.ToTable();
                    //updateBlockData(dicItemNo);
                    //---------------------------
                    try
                    {
            ...
            if (ExcelOutput)
            {
                ...
                try
                {
                    ...
                    else if(strText.Equals("PrintBOM", StringComparison.InvariantCultureIgnoreCase))
                    {
                        desc = strBomName;
                    }
                    exc.Writehead(dtnewExcel, System.IO.Path.GetFileNameWithoutExtension(doc.Name), sheetvalue, desc, pN);
        ...
        public bool timerollupfunctionality(out string sumresult, out string dwgNumber, out string SurfaceAreaSum, out string projectDesc, bool writeexcel = true, bool readDesc = true, bool divideBy60 = true, bool updateDWGAttr = true, string prevDesc = "",string strTotalTime="")
        {
            sumresult = "";
            dwgNumber = "";
            string revision = "";
            ...
            int maxCharAllowed = (string.IsNullOrEmpty(maxChar)) ? 0 : Convert.ToInt16(maxChar);
            _frm.maxAllowed = maxCharAllowed;
            ...
            if (!strText.Equals("GenerateProdDWG", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                projectDesc = _frm.textBoxdesc.Text;
            }
            else
            {
                _frm.readresult = true;
                _frm.finalTimeFromUI = Convert.ToDouble(strTotalTime);
                projectDesc = prevDesc;
                _frm.timeRollupCol = timeRollupCol;
            }
            readDesc = false;
            //if (result != true)
            //  return false;
        ...
        public bool timerollupfunctionalityWithoutUI(System.Collections.ObjectModel.ObservableCollection<SSDL.View.clsTimeRollUp> timeRollupCol, double finalTimeFromUI, out string sumresult, out string dwgNumber, out string SurfaceAreaSum, out string projectDesc, bool writeexcel = true, bool readDesc = true, bool divideBy60 = true, bool updateDWGAttr = true)
        {
            sumresult = "";
            dwgNumber = "";
            string revision = "";
            ...
            tempSum = finalTimeFromUI.ToString();
